...
extends Module {
    ...
    public WTap() {
        super("WTap", 0, Category.MOVEMENT, "Automatically taps W.");
    }
    ...
    @SubscribeEvent
    public void t3s4f(TickEvent.RenderTickEvent renderTickEvent) {
        if (this.updateData.hasReached(1000.0)) {
            this.setData();
        ...
        if (this.tHold.hasReached(this.hold) && this.shouldTap && Keyboard.isKeyDown((int)WTap.mc.gameSettings.keyBindForward.getKeyCode())) {
            KeyBinding.setKeyBindState((int)WTap.mc.gameSettings.keyBindForward.getKeyCode(), (boolean)true);
            this.shouldTap = false;
        } else if (!Keyboard.isKeyDown((int)WTap.mc.gameSettings.keyBindForward.getKeyCode())) {
